...
def run_pytest_scenario(scenario: str = None) -> bool:
    ...
    if scenario:
        ...
        if scenario in scenario_map:
            ...
            print(
                f"Error: Unknown scenario {scenario}. Valid scenarios: {list(scenario_map.keys())}"
            )
            return False
...
def main():
    ...
    parser.add_argument(
        "--config",
        help="Configuration file (currently ignored - uses built-in test configs)",
    )
    parser.add_argument(
        "--all", action="store_true", help="Run all validation scenarios"
    )
    parser.add_argument(
        "--output", help="Output directory (currently ignored - pytest handles output)"
    )
    ...
    if args.config:
        print(
            "Note: --config option not yet implemented. Using built-in test configurations."
        )
