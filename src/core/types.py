...
@dataclass
class Position:
    """Represents a 2D position on the spatial grid.
    
    Used for agent positioning, movement tracking, and marketplace detection.
    Positions are discrete grid coordinates with (0,0) at top-left.
...
class SimulationState:
    ...
    def __post_init__(self):
        ...
        if self.round_number < 0:
            raise ValueError(f"round_number must be non-negative, got {self.round_number}")
        if self.marketplace_participants < 0:
            raise ValueError(f"marketplace_participants must be non-negative, got {self.marketplace_participants}")
...
class MarketResult:
    ...
    def __post_init__(self):
        ...
        if len(self.unmet_demand) != n_goods:
            raise ValueError(f"unmet_demand length {len(self.unmet_demand)} != n_goods {n_goods}")
        if len(self.unmet_supply) != n_goods:
            raise ValueError(f"unmet_supply length {len(self.unmet_supply)} != n_goods {n_goods}")
        if len(self.total_volume) != n_goods:
            raise ValueError(f"total_volume length {len(self.total_volume)} != n_goods {n_goods}")
        if self.participant_count < 0:
            raise ValueError(f"participant_count must be non-negative, got {self.participant_count}")
