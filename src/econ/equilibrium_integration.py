...
def compare_solver_performance(
    agents: List, initial_guess: Optional[np.ndarray] = None, num_trials: int = 5
) -> dict:
    """
    ...
    for trial in range(num_trials):
        try:
            ...
            prices, z_norm, walras_dot, status = solve_walrasian_equilibrium(
                agents, initial_guess
            )
    ...
    for trial in range(num_trials):
        try:
            start_time = time.time()
            prices, z_norm, walras_dot, status = solve_walrasian_equilibrium_robust(
                agents, initial_guess
            )
            solve_time = time.time() - start_time
