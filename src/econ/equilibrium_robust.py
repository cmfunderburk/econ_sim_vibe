...
class RobustEquilibriumSolver:
    ...
    def solve_equilibrium_robust(
        ...
        if not agents:
            ...
            if return_diagnostics:
                diagnostics = ConvergenceResult(
                    False, "none", 0, np.inf, np.inf, 0.0, "no_participants", []
                )
        ...
        try:
            ...
            if success:
                ...
                prices = self._project_to_positive_orthant(
                    prices, preserve_numeraire=True
                )
                convergence_time = time.time() - start_time
                z_rest_norm, walras_dot = self._compute_convergence_metrics(
                    prices, viable_agents
                )
        ...
        try:
            ...
            if success:
                ...
                if z_rest_norm < self.tolerance:
                    logger.info(
                        f"Broyden's method converged in {iterations} iterations"
                    )
                    if return_diagnostics:
                        diagnostics = ConvergenceResult(
                            True,
        ...
        try:
            ...
            if success:
                ...
                if (
                    z_rest_norm < self.tolerance * 10
                ):  # Relaxed tolerance for tâtonnement
                    logger.info(f"Tâtonnement converged in {iterations} iterations")
                    status = (
                        "converged"
                        if z_rest_norm < self.tolerance
                        else "poor_convergence"
                    )
                    if return_diagnostics:
                        diagnostics = ConvergenceResult(
                            True,
    ...
    def _filter_viable_agents(self, agents: List, prices: np.ndarray) -> List:
        ...
        for agent in agents:
            ...
            if wealth > FEASIBILITY_TOL:
                ...
                logger.debug(
                    f"Agent {agent.agent_id} excluded with wealth {wealth:.2e}"
                )
        return viable_agents

    def _project_to_positive_orthant(
        self, prices: np.ndarray, preserve_numeraire: bool = True
    ) -> np.ndarray:
        """
    ...
    def _compute_analytical_jacobian(
        self, p_rest: np.ndarray, agents: List, n_goods: int
    ) -> np.ndarray:
        """
    ...
    def _newton_raphson_with_jacobian(
        self, excess_demand_func, initial_guess: np.ndarray, agents: List, n_goods: int
        ...
        for iteration in range(self.max_iterations):
            ...
            try:
                ...
                projected_prices = self._project_to_positive_orthant(
                    full_prices_candidate, preserve_numeraire=True
                )
    ...
    def _broyden_method(
        self, excess_demand_func, initial_guess: np.ndarray
    ) -> Tuple[np.ndarray, int, bool]:
        ...
        try:
            ...
            if result.success:
                ...
                projected_prices = self._project_to_positive_orthant(
                    full_prices, preserve_numeraire=True
                )
    ...
    def _compute_convergence_metrics(
        self, prices: np.ndarray, agents: List
    ) -> Tuple[float, float]:
...
def solve_walrasian_equilibrium_robust(
    agents: List,
    initial_guess: Optional[np.ndarray] = None,
