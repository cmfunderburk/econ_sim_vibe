...
class Grid:
    ...
    def __init__(self, width: int, height: int, marketplace_width: int = 2, marketplace_height: int = 2):
    ...
    def is_in_marketplace(self, position: Position) -> bool:
        ...
        """
        return (self.marketplace_x <= position.x < self.marketplace_x + self.marketplace_width and
                self.marketplace_y <= position.y < self.marketplace_y + self.marketplace_height)
    ...
    def get_agents_in_marketplace(self) -> Set[int]:
        ...
        """
        return {agent_id for agent_id, pos in self.agent_positions.items() 
    ...
    def distance_to_marketplace(self, position: Position) -> int:
        ...
        min_dist = float('inf')
        for mx in range(self.marketplace_x, self.marketplace_x + self.marketplace_width):
            for my in range(self.marketplace_y, self.marketplace_y + self.marketplace_height):
    ...
    def get_grid_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'marketplace_bounds': (
                self.marketplace_x, self.marketplace_y,
                self.marketplace_width, self.marketplace_height
            ),
...
def create_random_positions(n_agents: int, grid_width: int, grid_height: int, seed: Optional[int] = None) -> List[Position]:
