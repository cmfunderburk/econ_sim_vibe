...
strict_economic = pytest.mark.parametrize(
    "", [pytest.param(marks=[economic_core, real_functions])]
)
stress_test = pytest.mark.parametrize(
    "", [pytest.param(marks=[robustness, real_functions])]
)
...
def categorize_existing_test(test_name: str, test_description: str) -> list[str]:
    ...
    if any(
        keyword in test_name.lower() or keyword in test_description.lower()
