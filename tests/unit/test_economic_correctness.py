...
class TestBudgetConstrainedOrderGeneration:
    ...
    def test_scaling_when_budget_exceeded(self):
        ...
        max_sell_value = np.dot(
            prices, np.maximum(-theoretical_net, 0)
        )  # Value of what agent can sell
        ...
        if order.buy_orders[0] > 0 and order.buy_orders[1] > 0:
            ratio = (order.buy_orders[0] / prices[0]) / (
                order.buy_orders[1] / prices[1]
            )
        ...
        print(
            f"✅ Budget scaling working: theoretical buy value would be {np.dot(prices, np.maximum(theoretical_net, 0)):.3f}, actual is {buy_value:.3f}"
        )
    ...
    def test_inventory_constraints_respected(self):
        ...
        print(
            f"✅ Inventory constraints respected: max sells {order.sell_orders} ≤ personal {personal_endowment}"
        )
    ...
    def test_economic_consistency_with_equilibrium(self):
        ...
        assert (
            abs(total_buy_value - total_sell_value) < len(agents) * FEASIBILITY_TOL
        ), (
            f"Market imbalance too large: buy_value={total_buy_value}, sell_value={total_sell_value}"
        )

        print(
            f"✅ Market consistency: total buy value {total_buy_value:.3f} ≈ total sell value {total_sell_value:.3f}"
        )
...
class TestTheoryVsPracticeComparison:
    ...
    def test_pure_theory_vs_constrained_orders(self):
        ...
        print(
            f"PURE THEORY: Would want to buy {theoretical_buy_value:.2f} value, sell {theoretical_sell_value:.2f} value"
        )
        ...
        print(
            f"CONSTRAINED PRACTICE: Actually buys {actual_buy_value:.2f} value, sells {actual_sell_value:.2f} value"
        )
        print(
            f"PRACTICE SOLUTION: Buy value ≤ sell value, satisfies budget constraint!"
        )
        ...
        if order.buy_orders[0] > 0:
            ...
            print(
                f"Preference preservation: buy ratio {buy_ratio:.2f} vs preference ratio {preference_ratio:.2f}"
            )
