...
class TestPureExchangeBudgetConstraints:
    ...
    def test_budget_constraint_scaling_when_buy_exceeds_sell_value(self):
        ...
        unscaled_buy = np.maximum(unscaled_demand - agent.personal_endowment, 0.0)
        ...
        if unscaled_buy_value > sell_value + FEASIBILITY_TOL:
            ...
            assert np.any(agent_orders.buy_orders < unscaled_buy - FEASIBILITY_TOL), (
...
class TestRationingDiagnostics:
        ...
        for agent_id in [1, 2]:
            for good_id in range(2):
                if diagnostics.agent_unmet_buys[agent_id][good_id] > FEASIBILITY_TOL:
                    assert diagnostics.agent_fill_rates_buy[agent_id][good_id] < 1.0, (
                        f"Non-zero unmet buy orders should have fill rate < 100%"
                    )
                if diagnostics.agent_unmet_sells[agent_id][good_id] > FEASIBILITY_TOL:
                    assert diagnostics.agent_fill_rates_sell[agent_id][good_id] < 1.0, (
    ...
    def test_excess_demand_tracking(self):
        ...
        assert excess_demand_good2 > FEASIBILITY_TOL, (
            "Should have excess demand for good 2"
        )
...
class TestTravelCostIntegration:
    ...
    def test_travel_cost_reduces_effective_wealth(self):
        ...
        assert expected_adjusted_wealth == 16.0, (
            f"Travel cost should reduce wealth from {base_wealth} to 16"
        )  # Expected demand with adjusted wealth
    ...
    def test_travel_cost_can_reduce_wealth_to_zero(self):
        ...
        expected_demand = (
            ...
            else agent.alpha * adjusted_wealth / prices
        )
        np.testing.assert_allclose(
            expected_demand,
            np.zeros(2),
...
class TestMarketResultIntegration:
    ...
    def test_market_result_contains_all_diagnostics(self):
        ...
        assert hasattr(result, "executed_trades"), (
            "MarketResult missing executed_trades"
        )
        ...
        assert hasattr(result, "participant_count"), (
            "MarketResult missing participant_count"
        )
        assert hasattr(result, "rationing_diagnostics"), (
            "MarketResult missing rationing_diagnostics"
        )
        ...
        assert hasattr(diagnostics, "agent_unmet_buys"), (
            "Diagnostics missing agent_unmet_buys"
        )
        assert hasattr(diagnostics, "agent_unmet_sells"), (
            "Diagnostics missing agent_unmet_sells"
        )

        # Check that all agents are represented in diagnostics
        for agent_id in [1, 2]:
            assert agent_id in diagnostics.agent_unmet_buys, (
                f"Agent {agent_id} missing from agent_unmet_buys"
            )
            assert agent_id in diagnostics.agent_unmet_sells, (
                f"Agent {agent_id} missing from agent_unmet_sells"
            )
            assert agent_id in diagnostics.agent_fill_rates_buy, (
                f"Agent {agent_id} missing from agent_fill_rates_buy"
            )
            assert agent_id in diagnostics.agent_fill_rates_sell, (
                f"Agent {agent_id} missing from agent_fill_rates_sell"
            )

        # Check that all goods are represented
        n_goods = len(prices)
        assert len(diagnostics.good_demand_excess) == n_goods, (
            "Incorrect number of goods in good_demand_excess"
        )
