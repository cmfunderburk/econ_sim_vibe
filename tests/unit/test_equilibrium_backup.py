...
class TestComputeExcessDemand:
    ...
    def test_numeraire_constraint_validation(self):
        ...
        agent = Agent(
            1, np.array([0.5, 0.5]), np.array([1.0, 1.0]), np.array([0.0, 0.0])
        )
    ...
    def test_zero_wealth_agent_handling(self):
        ...
        zero_agent = Agent(
            1, np.array([0.5, 0.5]), np.array([0.0, 0.0]), np.array([0.0, 0.0])
        )
        normal_agent = Agent(
            2, np.array([0.5, 0.5]), np.array([1.0, 1.0]), np.array([0.0, 0.0])
        )

        # Should work with warning logged (zero wealth agent excluded)
        excess_demand = compute_excess_demand(
            np.array([1.0, 1.0]), [zero_agent, normal_agent]
        )
        assert isinstance(excess_demand, np.ndarray)
    ...
    def test_cobb_douglas_demand_formula(self):
        ...
        agent = Agent(
            1, np.array([0.6, 0.4]), np.array([1.0, 1.0]), np.array([0.0, 0.0])
        )
...
class TestSolveWalrasianEquilibrium:
    ...
    def test_zero_wealth_agent_filtering(self):
        ...
        zero_agent = Agent(
            2, np.array([0.5, 0.5]), np.array([0.0, 0.0]), np.array([0.0, 0.0])
        )

        prices, z_rest_norm, walras_dot, status = solve_walrasian_equilibrium(
            [viable_agent, zero_agent]
        )
    ...
    def test_initial_guess_handling(self):
        ...
        prices2, _, _, status2 = solve_walrasian_equilibrium(
...
class TestEconomicInvariants:
    ...
    def test_walras_law_always_holds(self):
        ...
        test_cases = [
            ...
            [
                ...
                Agent(
                    2, np.array([0.2, 0.8]), np.array([0.0, 2.0]), np.array([0.0, 0.0])
                ),
            ],
            # Case 3: Three agents
            [
                ...
                Agent(
                    3, np.array([0.5, 0.5]), np.array([1.0, 1.0]), np.array([0.0, 0.0])
                ),
            ],
        ]
    ...
    def test_conservation_of_total_endowments(self):
        ...
        agent1 = Agent(
            1, np.array([0.6, 0.4]), np.array([0.8, 0.1]), np.array([0.2, 0.1])
        )
        agent2 = Agent(
            2, np.array([0.3, 0.7]), np.array([0.1, 0.8]), np.array([0.1, 0.2])
        )
        ...
        total_initial = sum(
            agent.home_endowment + agent.personal_endowment for agent in agents
        )
...
class TestErrorHandlingAndRobustness:
    ...
    def test_large_endowment_disparities(self):
        ...
        agent1 = Agent(
            1, np.array([0.5, 0.5]), np.array([1000.0, 0.0]), np.array([0.0, 0.0])
        )
        agent2 = Agent(
            2, np.array([0.5, 0.5]), np.array([0.0, 0.001]), np.array([0.0, 0.0])
        )
    ...
    def test_numerical_precision_edge_cases(self):
        ...
        agent1 = Agent(
            1, np.array([0.5, 0.5]), np.array([1e-10, 0.0]), np.array([0.0, 0.0])
        )
        agent2 = Agent(
            2, np.array([0.5, 0.5]), np.array([0.0, 1e-10]), np.array([0.0, 0.0])
        )
