...
class TestWalrasLawAnalyticalValidation:
    ...
    def test_equilibrium_walras_law_enforcement(self):
        ...
        agent1 = Agent(
            1, np.array([0.6, 0.4]), np.array([0.5, 0.0]), np.array([0.5, 0.0])
        )
        agent2 = Agent(
            2, np.array([0.3, 0.7]), np.array([0.0, 0.5]), np.array([0.0, 0.5])
        )
...
class TestEquilibriumRobustness:
    ...
    def test_corner_solution_handling(self):
        ...
        agent1 = Agent(
            1, np.array([0.95, 0.05]), np.array([0.0, 1.0]), np.array([0.0, 0.0])
        )
        agent2 = Agent(
            2, np.array([0.05, 0.95]), np.array([1.0, 0.0]), np.array([0.0, 0.0])
        )
        ...
        assert status in [
            "converged",
            "poor_convergence",
        ]  # May struggle with extreme case
...
class TestBudgetConstraintEnforcement:
    ...
    def test_agent_demand_budget_feasibility(self):
        ...
        agent = Agent(
            1, np.array([0.6, 0.4]), np.array([2.0, 1.0]), np.array([1.0, 0.5])
        )
        prices = np.array([1.0, 3.0])
    ...
    def test_market_value_conservation(self):
        ...
        total_demand_value = sum(
            np.dot(prices, agent.demand(prices)) for agent in agents
        )
