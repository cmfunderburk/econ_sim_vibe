...
def test_logging_schema_guard_edgeworth():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        meta_path = out_dir / f"{result['config']['name']}_seed{result['config']['random_seed']}_metadata.json"
        ...
        for r in rows:
            ...
            assert r['core_schema_version'] == sidecar_version, "Schema version mismatch"
            assert r['financing_mode'] == 'PERSONAL', "financing_mode must be 'PERSONAL'"
        ...
        for rd in rounds:
            ...
            assert len(per_round) == len(agent_ids), f"Round {rd} has incomplete agent rows"
