...
class TestGenerateAgentOrders:
    ...
    def setup_method(self):
        ...
        self.agent1 = Agent(
            ...
            personal_endowment=np.array([1.0, 0.5]),  # Start with some personal inventory
    ...
    def test_order_inventory_consistency(self):
        ...
        for i, order in enumerate(orders):
            ...
            assert np.all(order.sell_orders <= order.max_sell_capacity + FEASIBILITY_TOL)
...
class TestComputeMarketTotals:
    ...
    def test_basic_totals_computation(self):
        ...
        orders = [
            AgentOrder(1, np.array([2.0, 0.5]), np.array([0.0, 1.0]), np.array([1.0, 2.0])),
            AgentOrder(2, np.array([1.0, 1.5]), np.array([0.5, 0.0]), np.array([1.0, 1.0]))
        ]
    ...
    def test_single_agent_totals(self):
        ...
        orders = [
            AgentOrder(1, np.array([1.5, 2.0]), np.array([0.5, 0.0]), np.array([1.0, 1.0]))
        ]
...
class TestProportionalRationing:
    ...
    def setup_method(self):
        ...
        self.orders = [
            AgentOrder(1, np.array([3.0, 1.0]), np.array([0.0, 2.0]), np.array([1.0, 3.0])),
            AgentOrder(2, np.array([1.0, 2.0]), np.array([1.0, 0.0]), np.array([2.0, 1.0]))
        ]
    ...
    def test_capacity_constraints(self):
        ...
        expected_volumes = np.minimum(np.minimum(self.total_buys, self.total_sells), capacity)
...
class TestClearingInvariantsValidation:
    ...
    def setup_method(self):
        ...
        self.orders = [
            AgentOrder(1, np.array([2.0, 1.0]), np.array([0.0, 1.5]), np.array([1.0, 2.0])),
            AgentOrder(2, np.array([1.0, 0.5]), np.array([1.0, 0.0]), np.array([1.5, 1.0]))
        ]
    ...
    def test_valid_invariants_pass(self):
        ...
        _validate_clearing_invariants(
            self.orders, valid_executed_buys, valid_executed_sells, 
            valid_executed_volumes, self.prices
        )
    ...
    def test_market_imbalance_detection(self):
        ...
        with pytest.raises(AssertionError, match="Volume inconsistency"):
            _validate_clearing_invariants(
                self.orders, self.executed_buys, self.executed_sells,
                imbalanced_volumes, self.prices
            )
    ...
    def test_inventory_constraint_violation(self):
        ...
        violating_sells = {
            ...
            2: np.array([1.5, 0.0])   # Agent 2 sells 1.5 > max_capacity[0]=1.5 (boundary case)
        }
        ...
        violating_orders = [
            AgentOrder(1, np.array([2.0, 1.0]), np.array([0.0, 1.5]), np.array([1.0, 2.0])),
            AgentOrder(2, np.array([1.0, 0.5]), np.array([1.0, 0.0]), np.array([1.0, 1.0]))  # Lower capacity
        ]
        
        # This should catch inventory constraint violation
        with pytest.raises(AssertionError, match="inventory constraint violated"):
            _validate_clearing_invariants(
                violating_orders, valid_buys, violating_sells,
                balanced_volumes, self.prices
            )
    
    def test_value_feasibility_violation(self):
        """Test detection of value feasibility violations."""
        # Create balanced market but value-infeasible for one agent
        violating_buys = {
            1: np.array([2.0, 1.0]),  # Agent 1 buys 2*1 + 1*2 = 4 value
            2: np.array([0.0, 0.0])   # Agent 2 no buys
        }
        violating_sells = {
            1: np.array([0.0, 0.5]),  # Agent 1 sells 0.5*2 = 1 value (buy > sell!)
            2: np.array([2.0, 0.5])   # Agent 2 sells to balance market
        }
        balanced_volumes = np.array([2.0, 1.0])  # Market balanced: total buys = total sells
        
        with pytest.raises(AssertionError, match="value infeasible"):
            _validate_clearing_invariants(
                self.orders, violating_buys, violating_sells,
                balanced_volumes, self.prices
            )
...
class TestExecuteConstrainedClearing:
    ...
    def test_input_validation(self):
        ...
        with pytest.raises(ValueError, match="capacity length"):
            execute_constrained_clearing(self.agents, self.prices, 
                                       capacity=np.array([1.0]))  # Wrong length
...
class TestApplyTradesToAgents:
    ...
    def test_successful_trade_application(self):
        ...
        assert np.isclose(self.agent1.personal_endowment[0], initial_personal_1[0] + 0.5)
        assert np.isclose(self.agent1.personal_endowment[1], initial_personal_1[1] - 0.5)
        assert np.isclose(self.agent2.personal_endowment[0], initial_personal_2[0] - 0.5)
        assert np.isclose(self.agent2.personal_endowment[1], initial_personal_2[1] + 0.5)
...
class TestMarketClearingIntegration:
    ...
    def test_edgeworth_clearing_integration(self):
        ...
        # Verify meaningful trade occurred
        assert len(result.executed_trades) > 0
        assert result.clearing_efficiency > 0.5  # Reasonable efficiency
