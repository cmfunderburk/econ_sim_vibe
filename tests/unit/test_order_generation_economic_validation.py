...
class TestOrderGenerationEconomicContent:
    ...
    def test_wealth_calculation_in_orders(self):
        ...
        reconstructed_wealth_good1 = (
            (analytical_excess[0] + agent.personal_endowment[0])
            * prices[0]
            / agent.alpha[0]
        )
        reconstructed_wealth_good2 = (
            (analytical_excess[1] + agent.personal_endowment[1])
            * prices[1]
            / agent.alpha[1]
        )
    ...
    def test_order_splits_are_economically_meaningful(self):
        ...
        assert net_order_good1 > 0, (
            f"Agent should be net buyer of good 1, got net order {net_order_good1}"
        )

        # Agent should want to sell good 2 (net negative order for good 2)
        net_order_good2 = order.buy_orders[1] - order.sell_orders[1]
        assert net_order_good2 < 0, (
            f"Agent should be net seller of good 2, got net order {net_order_good2}"
        )

        # Buy orders should be non-negative
        assert np.all(order.buy_orders >= 0), (
            f"Buy orders should be non-negative: {order.buy_orders}"
        )

        # Sell orders should be non-negative
        assert np.all(order.sell_orders >= 0), (
            f"Sell orders should be non-negative: {order.sell_orders}"
        )
...
class TestOrderGenerationEdgeCases:
    ...
    def test_zero_price_handling(self):
        ...
        assert np.all(np.isfinite(order.buy_orders)), (
            f"Buy orders should be finite: {order.buy_orders}"
        )
        assert np.all(np.isfinite(order.sell_orders)), (
            f"Sell orders should be finite: {order.sell_orders}"
        )
