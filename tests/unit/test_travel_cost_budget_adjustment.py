...
class TravelCostBudgetHelper:
    ...
    @staticmethod
    def compute_travel_cost_adjusted_wealth(agent: Agent, prices: np.ndarray, travel_cost: float) -> float:
        """
    ...
    @staticmethod
    def compute_demand_with_travel_cost(agent: Agent, prices: np.ndarray, travel_cost: float) -> np.ndarray:
        """
...
class TestTravelCostBudgetAdjustment:
    ...
    def test_zero_travel_cost_equals_normal_demand(self):
        ...
        assert np.allclose(demand_with_zero_cost, normal_demand, atol=FEASIBILITY_TOL), (
    ...
    def test_excessive_travel_cost_zero_demand(self):
        ...
        demand_with_excessive_cost = TravelCostBudgetHelper.compute_demand_with_travel_cost(
            agent, prices, excessive_travel_cost
        )
        
        expected_zero_demand = np.zeros_like(prices)
        
        assert np.allclose(demand_with_excessive_cost, expected_zero_demand, atol=FEASIBILITY_TOL), (
...
class TestTravelCostEdgeCases:
    ...
    def test_near_zero_adjusted_wealth(self):
        ...
        assert demand_magnitude < 1e-5, f"Demand should be very small: magnitude {demand_magnitude}"
    ...
    def test_multiple_agents_travel_cost_consistency(self):
        ...
        for agent in agents:
            ...
            assert np.all(demand >= 0), f"Agent {agent.agent_id} demand should be non-negative: {demand}"
        ...
        assert np.all(np.isfinite(total_adjusted_demand)), "Total demand should be finite"
...
class TestTravelCostIntegration:
    ...
    def test_travel_cost_welfare_implications(self):
        ...
        assert efficiency_loss >= -FEASIBILITY_TOL, "Should have non-negative efficiency loss"