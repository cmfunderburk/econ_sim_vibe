...
def test_v7_empty_marketplace_real_function_calls():
    ...
    print(f"   Function result: {len(result.executed_trades)} trades, {result.participant_count} participants")
    
    # Verify actual function behavior
    assert result.__class__.__name__ == "MarketResult", f"Expected MarketResult, got {type(result)}"
    ...
    assert result.clearing_efficiency == 1.0, f"Expected perfect efficiency with no demand, got {result.clearing_efficiency}"
    ...
    marketplace_participants = [agent for agent in agents_away if agent.position == (0, 0)]
    
    print(f"   Marketplace participants after filtering: {len(marketplace_participants)}")
    
    # Test equilibrium solver with filtered (empty) list
    prices_filtered, _, _, status_filtered = solve_walrasian_equilibrium(marketplace_participants)
    
    assert prices_filtered is None, f"Expected None prices with no marketplace participants"
    assert status_filtered == "no_participants", f"Expected 'no_participants', got '{status_filtered}'"
    ...
    assert status_single == "insufficient_participants", f"Expected 'insufficient_participants', got '{status_single}'"
...
def test_v8_stop_conditions_real_function_calls():
    ...
    if status == "converged":
        ...
        print(f"   Convergence metrics: z_norm={z_rest_norm:.2e}, walras={abs(walras_dot):.2e}")
        ...
        assert isinstance(result.clearing_efficiency, float), "Clearing efficiency should be computed"
    ...
    difficult_agents = [
        Agent(1, np.array([0.999, 0.001]), np.array([1.0, 0.0]), np.array([0.0, 0.0]), (0, 0)),
        Agent(2, np.array([0.001, 0.999]), np.array([0.0, 1.0]), np.array([0.0, 0.0]), (0, 0)),
    ]
    
    prices_diff, z_norm_diff, walras_diff, status_diff = solve_walrasian_equilibrium(difficult_agents)
    
    # Test actual convergence failure detection
    convergence_failed = (
        status_diff in ["failed", "poor_convergence"] or
        z_norm_diff >= SOLVER_TOL or
        abs(walras_diff) >= SOLVER_TOL
    )
    
    print(f"   Difficult case status: '{status_diff}'")
    print(f"   Convergence metrics: z_norm={z_norm_diff:.2e}, walras={abs(walras_diff):.2e}")
    ...
    assert status_diff in ["converged", "failed", "poor_convergence", "insufficient_participants"], (
    ...
    if status == "converged":
        ...
        current_utilities = [agent.utility(agent.personal_endowment) for agent in agents]
        
        # Test real progress measurement
        utility_changes = [abs(curr - prev) for curr, prev in zip(current_utilities, previous_utilities)]
        ...
        assert all(isinstance(u, (int, float)) for u in current_utilities), "Utilities should be computed"
